...
# Load v7um rules as baseline
with open('v7um_apex_master_30s_rules.json') as f:
    best_rules = json.load(f)

actions = ['SAME', 'OPP', 'LAST', 'OPP_LAST']

# Collect all active keys across all 9 feeds
active_keys = set()
for name, data in datasets:
    sizes = [d['size'] for d in data]
    for s in range(3):
...
print(f'Active keys across all 9 feeds: {len(active_keys)}')

w, l, ms, busts, l1, l2, l3, f_res = simulate(best_rules)
best_score = score(w, l, ms, busts, l1, l2, l3)
print(f'Starting with v7UM: Score={best_score:.1f}, Wins={w}/{w+l} ({w/(w+l)*100:.1f}%), MaxStreak={ms}, Busts={busts}, L1={l1}, L2={l2}, L3={l3}')

# Greedy multi-pass ascent
for it in range(25):
    improved = False
    for k in sorted(active_keys):
        st, c, p, p2, a, fl, cs = map(int, k.split('_'))
        orig = best_rules.get(k, 'SAME')
        ...
        for act in cand_actions:
            ...
            if sc > best_score:
                ...
                best_act = act
                improved = True
                print(f'Iter {it}: {k} -> {act} | Score={sc:.1f}, W={w}/{w+l} ({w/(w+l)*100:.1f}%), MS={ms}, Busts={busts}, L1={l1}, L2={l2}, L3={l3}')
        best_rules[k] = best_act
    if not improved:
        break

w, l, ms, busts, l1, l2, l3, f_res = simulate(best_rules)
print('\n================ ALL 9 FEEDS ZERO-BUST MASTER RESULTS ================')
...
with open('v8um_apex_zero_bust_30s_rules.json', 'w') as f:
    json.dump(best_rules, f, indent=2)

print('Saved v8um_apex_zero_bust_30s_rules.json successfully.')
