...
def test_pure_antichop():
    ...
    for name, seq in all_real_sequences:
        ...
        for item in seq[3:]:
            ...
            # PURE THEOREM
            if streak >= 2: # Level 3 Zero Loss Shield
                if alt >= 1:
                    pred = opp(last_s)
                    tag = "🛑 L3_CHOP_SHIELD"
                else:
...
tm, tw, tl, res = test_pure_antichop()
print("="*80)
print(f"PURE ANTI-CHOP THEOREM RESULTS: {tw}W / {tl}L ({(tw/(tw+tl))*100:.1f}%) | MAX CONSECUTIVE LOSSES = {tm}")
print("="*80)
