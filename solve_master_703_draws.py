...
print(f"Total Unique Feeds: {len(all_feeds)} | Total Draws: {total_draws}")
...
# Collect all optimizable active keys
all_active_keys = set()
for name, fd in all_feeds:
    if len(fd) < 4: continue
    for i in range(3, len(fd)):
...
w, r, pct, ms, b = init_res["wins"], init_res["rounds"], init_res["win_rate"], init_res["max_streak"], init_res["busts"]
print(f"Starting: Wins={w}/{r} ({pct:.1f}%), MS={ms}, Busts={b}")
...
print("\n" + "="*80)
print(f"FINAL ZERO-BUST MASTER RESULT ON ALL {len(all_feeds)} FEEDS ({total_draws} DRAWS):")
...
print("\nSaved master rules to v9um_apex_titan_supreme_9feature_rules.json successfully.")
