...
def extract_features_v8(sizes, streak):
    runs = get_runs(sizes)
    c_run = runs[-1]
    c_side = c_run['size']
    c_len = c_run['len']
    ...
    p3_run = runs[-4] if len(runs) >= 4 else {'size': opp(c_side), 'len': 0}
    last_s = sizes[-1]
    ...
    key = f'{streak_cat}_{c_len_cat}_{p_len_cat}_{p2_len_cat}_{p3_len_cat}_{alt_cat}_{flip_cat}_{c_side_bit}'
    return key, c_side, last_s, c_len, alt, p_run['len']

def simulate_v8(rules_dict):
    ...
    for name, data in datasets:
        ...
        for i in range(4, len(sizes)):
            key, c_side, last_s, c_len, alt, p_len = extract_features_v8(sizes[:i], streak)
...
def score_v8(w, l, ms, busts, l1, l2, l3):
    return (w * 20.0) - (l * 2.0) - (busts * 500000.0) - (max(0, ms - 2) * 2000000.0) + (l1 * 15.0) + (l2 * 5.0)
...
for name, data in datasets:
    ...
    for s in range(3):
        for i in range(4, len(sizes)):
            k, _, _, _, _, _ = extract_features_v8(sizes[:i], s)
            active_keys.add(k)
            st, c, p, p2, p3, a, fl, cs = map(int, k.split('_'))
            if c >= 2:
                rules[k] = 'SAME'
            elif a >= 2:
                rules[k] = 'OPP_LAST'
...
print(f'Active 8-feature keys across 274 draws: {len(active_keys)}')

w, l, ms, busts, l1, l2, l3, f_res = simulate_v8(rules)
best_score = score_v8(w, l, ms, busts, l1, l2, l3)
...
for it in range(15):
    ...
    for k in sorted(active_keys):
        ...
        for act in cand_actions:
            ...
            rules[k] = act
            w, l, ms, busts, l1, l2, l3, _ = simulate_v8(rules)
            sc = score_v8(w, l, ms, busts, l1, l2, l3)
...
w, l, ms, busts, l1, l2, l3, f_res = simulate_v8(rules)
print('\n================ 8-FEATURE v8UM ZERO-BUST RESULTS ================')
...
with open('v8um_8feat_master_30s_rules.json', 'w') as f:
