...
def score(w, l, ms, busts, l1, l2, l3):
    return (w * 20.0) - (l * 2.0) - (busts * 5000000.0) - (max(0, ms - 2) * 20000000.0) + (l1 * 10.0) + (l2 * 4.0)
...
print(f'Starting Score: {best_score:.1f}, Wins={w}/{w+l}, MS={ms}, Busts={busts}')

# Coordinate search with random restarts on tied moves
for iteration in range(50):
    improved = False
    shuffled_keys = list(active_keys)
    ...
    for k in shuffled_keys:
        ...
        for act in cand_actions:
            ...
            if sc > best_score:
                ...
                orig = act
                improved = True
                print(f'Iter {iteration}: {k} -> {act} | Score={sc:.1f}, W={w}/{w+l} ({w/(w+l)*100:.1f}%), MS={ms}, Busts={busts}')
        best_rules[k] = orig
    if not improved and ms <= 2 and busts == 0:
        break

w, l, ms, busts, l1, l2, l3, f_res = simulate(best_rules)
print('\n================ STRICT ZERO-BUST ON ALL 10 FEEDS (330 DRAWS) ================')
print(f'Total Across All 10 Feeds: Wins={w}/{w+l} ({w/(w+l)*100:.1f}%), MaxStreak={ms}, Busts={busts}')
...
print('Saved perfect zero-bust v9um_apex_titan_supreme_30s_rules.json successfully.')
