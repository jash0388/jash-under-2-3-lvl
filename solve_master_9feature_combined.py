...
# 1. Load historical master feeds
with open("all_master_combined_574.json") as f:
    master_574 = json.load(f)

# 2. Load latest user feed (350 draws from 51028 to 51668)
...
print(f"Total Combined Real Market Draws: {total_draws}")
for name, fd in all_feeds:
    print(f" - {name}: {len(fd)} draws")
...
def simulate_engine(rules):
    ...
    # High penalty for loss streak > 2, maximize wins
...
# Start with existing 9-feature rules
...
for iteration in range(6):
    improved = False
    keys_list = list(all_active_keys)
