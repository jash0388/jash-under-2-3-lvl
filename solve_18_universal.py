from dataset_18 import all_18_seqs, get_runs, opp
import itertools

seqs_30s = [s for s in all_18_seqs if '30S' in s[0]]
seqs_1m = [s for s in all_18_seqs if '1M' in s[0]]

print(f"30S sequences: {len(seqs_30s)} sequences ({sum(len(s[1]) for s in seqs_30s)} draws)")
for name, s in seqs_30s:
    print(f"  - {name}: {len(s)} draws")
print(f"\n1M sequences: {len(seqs_1m)} sequences ({sum(len(s[1]) for s in seqs_1m)} draws)")
    ...
    def make_fn_30s(params):
        ...
        def pred_30s(hist, streak):
            ...
            if streak >= 2: # L3
                ...
                elif alt >= 3: return (opp(last_s) if p2_c3 == 'OSC' else (opp(last_s) if p2_c3 == 'FLIP' else c_side)), "L2_C3"
...
print(f"\n=======================================================")
print(f"30S MODE: Found {len(best_30s)} engines with MAX LOSS <= 2 across ALL 7 30S sequences!")
if best_30s:
    best_30s.sort(key=lambda x: (x[2], -x[0])) # prioritize lowest max streak, highest wins
    ...
    for k, v in zip(labels, bp):
        print(f"  {k}: '{v}',")
    print("\n30S Breakdown across all 7 real market sequences:")
    ...
    print("❌ No parameter combination found with Max Loss <= 2. Expanding search...")
