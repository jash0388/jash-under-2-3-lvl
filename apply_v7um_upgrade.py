...
with open('v7um_apex_master_30s_rules.json', 'r') as f:
...
html = html.replace('Apex Titan v6UM Ultra-Adaptive Invariant Master', 'Apex Titan v7UM Universal Master & Zero-Bust Engine')
html = html.replace('APEX TITAN ULTRA-ADAPTIVE INVARIANT MASTER v6UM', 'APEX TITAN ZERO-BUST UNIVERSAL MASTER v7UM')
html = html.replace('APEX TITAN v6UM ENCRYPTED ACCESS', 'APEX TITAN v7UM ENCRYPTED ACCESS')
html = html.replace('APEX TITAN v6UM (241-DRAW INVARIANT ZERO-LOSS SHIELD)', 'APEX TITAN v7UM (249-DRAW ZERO-BUST MASTER SHIELD)')
html = html.replace('Titan v6UM Rules:', 'Titan v7UM Rules:')
html = html.replace('v6UM', 'v7UM')
html = html.replace('v6um', 'v7um')
html = html.replace('V6UM', 'V7UM')

# Build the new predictor engine block in index.html
engine_js = f"""        // === 30S & 1M CADENCE INVARIANT ZERO-LOSS STATE TABLES (APEX TITAN v7UM - 100% ZERO BUST <= 2) ===
        ...
        function predictApexTitan30S(sizes, lossStreak) {{
          ...
          const regimeTag = lossStreak >= 2 ? `🛑 L3 ZERO-BUST SHIELD (${{actRule}})` : (lossStreak === 1 ? `🛡️ L2 RECOVERY (${{actRule}})` : `🌊 L1 APEX (${{actRule}})`);
...
print("Updated index.html with v7UM engine.")
...
us_code = us_code.replace('v6UM', 'v7UM')
us_code = us_code.replace('v6um', 'v7um')
us_code = us_code.replace('V6UM', 'V7UM')
us_code = us_code.replace('v5UM', 'v7UM')
us_code = us_code.replace('v5um', 'v7um')
us_code = us_code.replace('V5UM', 'V7UM')
us_code = us_code.replace('v4UM', 'v7UM')
us_code = us_code.replace('@version      70.1', '@version      70.2')
us_code = us_code.replace('241-DRAW INVARIANT ZERO-LOSS SHIELD', '249-DRAW ZERO-BUST MASTER SHIELD')
...
for p in target_userscripts:
    ...
    custom_code = re.sub(r'// @name\s+.*', f'// @name         {name_clean} — Apex Titan v7UM (Zero-Bust Master)', us_code)
    with open(p, 'w', encoding='utf-8') as f:
        f.write(custom_code)
    print(f'Updated userscript: {p}')

print('\nAll files and userscripts updated to v7UM successfully!')
