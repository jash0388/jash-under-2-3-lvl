...
seqs_30s = [
    s for s in all_real_sequences if '30S' in s[0]
] + [
    ("Seq Q (30S 52480 - Live)", seq_Q),
    ("Seq R (30S 52524 - Live)", seq_R),
    ("Seq S (30S 52570-52590 Live Full)", seq_S_full)
]
...
def build_feature_key(hist, streak):
    ...
    recent = sizes[-5:] if len(sizes) >= 5 else sizes
    flips = sum(1 for i in range(1, len(recent)) if recent[i] != recent[i-1])
    flip_cat = 0 if flips <= 1 else (1 if flips == 2 else 2)
...
if best_sol:
    ...
    print(f"🏆 ZERO-BUST TITAN SOLVED: {tw}W / {tl}L ({(tw/(tw+tl))*100:.1f}%) | Max Streak = {tm}")
    ...
    with open('v1um_state_rules.json', 'w') as f:
        json.dump(state_dict, f, indent=2)
    print("Exported v1um_state_rules.json successfully!")
