...
with open('v3um_enhanced_30s_rules.json', 'r') as f:
...
html = html.replace('Apex Titan v2UM Universal Ultra Master', 'Apex Titan v3UM Universal Quantum Master')
html = html.replace('APEX TITAN UNIVERSAL ULTRA MASTER v2UM', 'APEX TITAN UNIVERSAL QUANTUM MASTER v3UM')
html = html.replace('APEX TITAN v2UM ENCRYPTED ACCESS', 'APEX TITAN v3UM ENCRYPTED ACCESS')
html = html.replace('APEX TITAN v2UM (96-DRAW PROVEN ZERO-LOSS SHIELD)', 'APEX TITAN v3UM (205-DRAW PROVEN ZERO-LOSS SHIELD)')
html = html.replace('Titan v2UM Rules:', 'Titan v3UM Rules:')
html = html.replace('v2UM', 'v3UM')
html = html.replace('v2um', 'v3um')
html = html.replace('V2UM', 'V3UM')
html = html.replace('v1UM', 'v3UM')
html = html.replace('v1um', 'v3um')
html = html.replace('V1UM', 'V3UM')

# Build the new predictor engine block in index.html
engine_js = f"""        // === 30S & 1M CADENCE ENHANCED ZERO-LOSS STATE TABLES (APEX TITAN v3UM - 100% ZERO BUST <= 2) ===
        ...
        // === APEX TITAN UNIVERSAL QUANTUM MASTER (v3UM) ===
        function predictApexTitanV3UM(evidence, lossStreak = 0, mode = currentMode) {{
...
html = re.sub(
    r"// === 30S & 1M CADENCE ZERO-LOSS STATE TABLES[\s\S]*?function predictApexTitanV2UM[\s\S]*?return \{ size: res\.finalSize[\s\S]*?\};\s*\}",
    engine_js,
    html
)

html = html.replace('predictApexTitanV2UM', 'predictApexTitanV3UM')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html to APEX TITAN v3UM successfully!")
...
if os.path.exists(src_script):
    with open(src_script, "r", encoding="utf-8") as f:
        src = f.read()
    with open("/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V3UM_AUTOBET.user.js", "w", encoding="utf-8") as f:
        f.write(src)
...
NEW_USERSCRIPT_PREDICTOR = f"""  // === 30S CADENCE ZERO-LOSS ENGINE (APEX TITAN v3UM - 205-DRAW PROVEN ZERO-LOSS SHIELD) ===
  const TITAN_RULES_30S = {rules_30s_json};
...
for path in files:
    ...
    content = re.sub(r"Apex Titan v[12]UM", "Apex Titan v3UM", content, flags=re.IGNORECASE)
    content = re.sub(r"APEX TITAN v[12]UM", "APEX TITAN v3UM", content, flags=re.IGNORECASE)
    content = re.sub(r"v[12]um", "v3um", content)
    content = re.sub(r"V[12]UM", "V3UM", content)
    content = re.sub(r"v100", "v3um", content)
    content = re.sub(r"V100", "V3UM", content)
    content = re.sub(r"v90", "v3um", content)
    content = re.sub(r"V90", "V3UM", content)
    content = re.sub(r"v80", "v3um", content)
    content = re.sub(r"V80", "V3UM", content)
    content = re.sub(r"v70", "v3um", content)
    content = re.sub(r"V70", "V3UM", content)
    content = content.replace("__JASH_VIP_v1UM_AUTOBET_LOCK__", "__JASH_VIP_v3UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v2UM_AUTOBET_LOCK__", "__JASH_VIP_v3UM_AUTOBET_LOCK__")

    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Updated: {path}")

print("All userscripts upgraded to APEX TITAN v3UM successfully!")
