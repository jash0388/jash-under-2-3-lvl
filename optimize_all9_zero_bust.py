...
def score(w, l, ms, busts, l1, l2, l3):
    return (w * 10.0) - (l * 1.0) - (busts * 200000.0) - (max(0, ms - 2) * 1000000.0) + (l1 * 8.0) + (l2 * 3.0)
                    ...
                    for fl in range(3):
                        for cs in [0, 1]:
                            ...
                            if c >= 2:
                                ...
                                rules[k] = 'SAME' # Doublet follow
                            else:
                                rules[k] = 'SAME'

# Find all active keys in all 9 datasets
active_keys = set()
for name, data in datasets:
    sizes = [d['size'] for d in data]
    for s in range(3):
...
print(f'Total active keys across all 9 feeds (274 draws): {len(active_keys)}')
...
for it in range(15):
    ...
    for k in sorted(active_keys):
        ...
        # Valid actions: enforce dragon lock for c >= 2 at recovery levels
        ...
        for act in cand_actions:
            ...
            if sc > best_score:
                ...
                best_act = act
                improved = True
                # print(f'Iter {it}: {k} -> {act} | W={w}/{w+l}, MS={ms}, Busts={busts}, L1={l1}')
        rules[k] = best_act
    if not improved:
        break

w, l, ms, busts, l1, l2, l3, f_res = simulate(rules)
print('\n================ FINAL v8UM ZERO-BUST RESULTS ================')
...
print('Saved v8um_supreme_master_30s_rules.json successfully.')
