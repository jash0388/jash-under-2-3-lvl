...
print("=== Starting 100% Strict /pred Web Signal Follower Build ===")

# 1. Update api/live-signal.js
api_live_signal_code = '''const fs = require('fs');
...
with open('api/live-signal.js', 'w') as f:
    f.write(api_live_signal_code)
print("Updated api/live-signal.js successfully.")
...
for pf in pred_files:
    ...
    # Fix actRule === "BIG" || actRule === "SMALL" in predictApexTitan30S and predictApexTitan1M
    content = re.sub(
        r'(function predictApexTitan30S.*?let finalSize = cSide;.*?else if \(actRule === "OPP_LAST"\) finalSize = opp\(lastS\);)(.*?const conf)',
        r'\1\n          else if (actRule === "BIG" || actRule === "SMALL") finalSize = actRule;\2',
        content,
        flags=re.DOTALL
    )
    content = re.sub(
        r'(function predictApexTitan1M.*?let finalSize = cSide;.*?else if \(actRule === "OPP_LAST"\) finalSize = opp\(lastS\);)(.*?const conf)',
        r'\1\n          else if (actRule === "BIG" || actRule === "SMALL") finalSize = actRule;\2',
        content,
        flags=re.DOTALL
    )

    # Ensure broadcast function exists and is called on render & snapshot
    broadcast_func = '''
        function broadcastLiveSignal(reqMode, targetPeriod, pred) {
          if (!pred || !targetPeriod || targetPeriod === "—") return;
          try {
...
'''
    if 'function broadcastLiveSignal' not in content:
        content = content.replace('function snapshotPendingPrediction(', broadcast_func + '\n        function snapshotPendingPrediction(')

    # Call broadcastLiveSignal in snapshotPendingPrediction
    content = re.sub(
        r'savePending\(pendingPrediction, reqMode\);.*?(// Broadcast live signal.*?\n(\s+fetch.*?\n)*\s+\}\s+catch\s*\(e\)\s*\{\})?',
        r'savePending(pendingPrediction, reqMode);\n          broadcastLiveSignal(reqMode, targetPeriod, pred);',
        content,
        flags=re.DOTALL
    )

    # Call broadcastLiveSignal in render()
    if 'broadcastLiveSignal(currentMode, state.targetPeriod, prediction);' not in content:
        content = content.replace('state.prediction = prediction;', 'state.prediction = prediction;\n          if (prediction && state.targetPeriod && state.targetPeriod !== "—") { broadcastLiveSignal(currentMode, state.targetPeriod, prediction); }')

    with open(pf, 'w') as f:
        f.write(content)
    print(f"Updated {pf} with broadcast & rule fixes.")
