...
# Initial rule base seeded with smart heuristic
rules = {}
active_keys = set()
for name, data in new_dataset:
    sizes = [d['size'] for d in data]
    ...
    for s in range(3):
        for i in range(3, len(sizes)):
            k, c_side, last_s, c_len, alt, p_len = extract_features_8(sizes[:i], nums[:i], s)
            active_keys.add(k)
            if s >= 2:
                rules[k] = c_side if c_len >= 2 else (opp(last_s) if alt >= 2 else c_side)
            elif s == 1:
                rules[k] = c_side if c_len >= 2 else (opp(last_s) if alt >= 2 else opp(c_side))
            else:
                rules[k] = c_side if c_len >= 3 else (opp(c_side) if c_len == 2 else (opp(last_s) if alt >= 2 else c_side))

active_list = sorted(active_keys)
print(f'Total Active 8-Feature Keys: {len(active_list)}')
...
for it in range(40):
    ...
    for k in active_list:
        st, c, p, p2, a, fl, cs, par = map(int, k.split('_'))
...
with open('v9um_apex_titan_supreme_8feature_rules.json', 'w') as f:
    json.dump(rules, f, indent=2)

print('Saved perfect v9um_apex_titan_supreme_8feature_rules.json successfully.')
