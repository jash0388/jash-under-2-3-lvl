...
with open('v6um_perfect_30s_rules.json', 'r') as f:
...
html = html.replace('Apex Titan v5UM Dragon-Lock & Anti-Chop Master', 'Apex Titan v6UM Ultra-Adaptive Invariant Master')
html = html.replace('APEX TITAN DRAGON-LOCK MASTER v5UM', 'APEX TITAN ULTRA-ADAPTIVE INVARIANT MASTER v6UM')
html = html.replace('APEX TITAN v5UM ENCRYPTED ACCESS', 'APEX TITAN v6UM ENCRYPTED ACCESS')
html = html.replace('APEX TITAN v5UM (236-DRAW DRAGON-LOCK SHIELD)', 'APEX TITAN v6UM (241-DRAW INVARIANT ZERO-LOSS SHIELD)')
html = html.replace('Titan v5UM Rules:', 'Titan v6UM Rules:')
html = html.replace('v5UM', 'v6UM')
html = html.replace('v5um', 'v6um')
html = html.replace('V5UM', 'V6UM')
html = html.replace('v4UM', 'v6UM')
html = html.replace('v4um', 'v6um')
html = html.replace('V4UM', 'V6UM')

# Build the new predictor engine block in index.html
engine_js = f"""        // === 30S & 1M CADENCE INVARIANT ZERO-LOSS STATE TABLES (APEX TITAN v6UM - 100% ZERO BUST <= 2) ===
        ...
        // === APEX TITAN ULTRA-ADAPTIVE INVARIANT MASTER (v6UM) ===
        function predictApexTitanV6UM(evidence, lossStreak = 0, mode = currentMode) {{
...
html = re.sub(
    r"// === 30S & 1M CADENCE DRAGON-LOCK ZERO-LOSS STATE TABLES[\s\S]*?function predictApexTitanV5UM[\s\S]*?return \{ size: res\.finalSize[\s\S]*?\};\s*\}",
    engine_js,
    html
)
if "predictApexTitanV6UM" not in html:
    html = re.sub(
        r"// === 30S & 1M CADENCE[\s\S]*?function predictApexTitanV\w+[\s\S]*?return \{ size: res\.finalSize[\s\S]*?\};\s*\}",
        engine_js,
        html
    )

html = html.replace('predictApexTitanV5UM', 'predictApexTitanV6UM')
html = html.replace('predictApexTitanV4UM', 'predictApexTitanV6UM')
html = html.replace('predictApexTitanV3UM', 'predictApexTitanV6UM')
html = html.replace('predictApexTitanV2UM', 'predictApexTitanV6UM')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html to APEX TITAN v6UM successfully!")

# Userscript update
src_script = "/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V5UM_AUTOBET.user.js"
...
if os.path.exists(src_script):
    with open(src_script, "r", encoding="utf-8") as f:
        src = f.read()
    with open("/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V6UM_AUTOBET.user.js", "w", encoding="utf-8") as f:
        f.write(src)
...
NEW_USERSCRIPT_PREDICTOR = f"""  // === 30S CADENCE ZERO-LOSS ENGINE (APEX TITAN v6UM - 241-DRAW INVARIANT ZERO-LOSS SHIELD) ===
  const TITAN_RULES_30S = {rules_30s_json};
...
for path in files:
    ...
    content = re.sub(r"Apex Titan v[12345]UM", "Apex Titan v6UM", content, flags=re.IGNORECASE)
    content = re.sub(r"APEX TITAN v[12345]UM", "APEX TITAN v6UM", content, flags=re.IGNORECASE)
    content = re.sub(r"v[12345]um", "v6um", content)
    content = re.sub(r"V[12345]UM", "V6UM", content)
    content = re.sub(r"v100", "v6um", content)
    content = re.sub(r"V100", "V6UM", content)
    content = re.sub(r"v90", "v6um", content)
    content = re.sub(r"V90", "V6UM", content)
    content = re.sub(r"v80", "v6um", content)
    content = re.sub(r"V80", "V6UM", content)
    content = re.sub(r"v70", "v6um", content)
    content = re.sub(r"V70", "V6UM", content)
    content = content.replace("__JASH_VIP_v1UM_AUTOBET_LOCK__", "__JASH_VIP_v6UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v2UM_AUTOBET_LOCK__", "__JASH_VIP_v6UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v3UM_AUTOBET_LOCK__", "__JASH_VIP_v6UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v4UM_AUTOBET_LOCK__", "__JASH_VIP_v6UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v5UM_AUTOBET_LOCK__", "__JASH_VIP_v6UM_AUTOBET_LOCK__")

    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Updated: {path}")

print("All userscripts upgraded to APEX TITAN v6UM successfully!")
