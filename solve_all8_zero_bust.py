...
# All 8 Real 30S Market Sequences
...
all_30s = [
    ...
    ("Seq T (30S 80052604 - Latest Live)", seq_T),
]

print(f"Loaded {len(all_30s)} 30S sequences with {sum(len(s[1]) for s in all_30s)} draws.")
...
if best_sol:
    ...
    print(f"🏆 ALL-8 ZERO-BUST TITAN SOLVED: {tw}W / {tl}L ({(tw/(tw+tl))*100:.1f}%) | Max Streak = {tm}")
    ...
    with open('v1um_all8_rules.json', 'w') as f:
        json.dump(state_dict, f, indent=2)
    print("Exported v1um_all8_rules.json successfully!")
