...
def get_prediction(rules, sizes, nums, loss_streak):
    ...
    # ── DRAGON & STRUCTURAL HARD INVARIANTS ──
    if c_len >= 3:
        # Never fight a dragon streak >= 3 rounds
        return c_side
...
for name, fd in all_feeds:
    ...
    for i in range(3, len(fd)):
        ...
        nums = [d["number"] for d in hist]
        for streak in [0, 1, 2]:
            key, _, _, c_len, _ = extract_features9(sizes, nums, streak)
            if c_len < 3: # c_len >= 3 is hard-coded Dragon Lock
                all_active_keys.add(key)

print(f"Total Active Optimizable Keys: {len(all_active_keys)}")

init_res = evaluate_rules(best_rules)
best_score = init_res["score"]
print(f"Starting Score={best_score}, Wins={init_res['wins']}/{init_res['rounds']} ({init_res['win_rate']:.1f}%), MS={init_res['max_streak']}, Busts={init_res['busts']}")
...
final_res = evaluate_rules(best_rules)
print("\n" + "="*80)
print(f"FINAL ZERO-BUST MASTER RESULT ON ALL 11 FEEDS ({sum(len(f[1]) for f in all_feeds)} DRAWS):")
...
print("\nSaved master rules to v9um_apex_titan_supreme_9feature_rules.json.")
