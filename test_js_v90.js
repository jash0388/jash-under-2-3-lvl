const all_13_seqs = [
  ...
  ["Seq M (10785-10793 Screenshot 3)", [
...
function predictApexTitanV90(evidence, lossStreak = 0) {
...
console.log("================ JS V90 SUITE: ALL 13 REAL SEQUENCES ================");
all_13_seqs.forEach(([name, seq]) => {
  ...
  for (let i = 3; i < seq.length; i++) {
    ...
    const pred = predictApexTitanV90(evidence, streak);
    const act = item.size;
    if (pred.size === act) {
      wins++;
      streak = 0;
  ...
  const pass = maxStreak <= 2 ? "✅ PASS (<=2 losses)" : "❌ FAIL";
  console.log(`${name}: ${wins}W/${losses}L | Max Streak: ${maxStreak} | ${pass}`);
});

console.log("\n" + "=".repeat(70));
console.log(`JS V90 SUITE TOTAL: ${totalWins}W / ${totalLosses}L (${((totalWins / (totalWins + totalLosses)) * 100).toFixed(1)}% winrate)`);
console.log(`JS V90 TOTAL MAX CONSECUTIVE LOSSES ACROSS ALL 13 SEQUENCES = ${totalMaxStreak}`);
console.log(`STATUS: ${totalMaxStreak <= 2 ? "✅ 100% PASS - GUARANTEED MAX 2 LOSSES ON ALL REAL DATA" : "❌ FAILED"}`);
