...
with open('v4um_supreme_30s_rules.json', 'r') as f:
...
html = html.replace('Apex Titan v3UM Universal Quantum Master', 'Apex Titan v4UM Supreme Master (Anti-Chop Shield)')
html = html.replace('APEX TITAN UNIVERSAL QUANTUM MASTER v3UM', 'APEX TITAN SUPREME MASTER v4UM (ANTI-CHOP SHIELD)')
html = html.replace('APEX TITAN v3UM ENCRYPTED ACCESS', 'APEX TITAN v4UM ENCRYPTED ACCESS')
html = html.replace('APEX TITAN v3UM (205-DRAW PROVEN ZERO-LOSS SHIELD)', 'APEX TITAN v4UM (213-DRAW SUPREME ZERO-LOSS SHIELD)')
html = html.replace('Titan v3UM Rules:', 'Titan v4UM Rules:')
html = html.replace('v3UM', 'v4UM')
html = html.replace('v3um', 'v4um')
html = html.replace('V3UM', 'V4UM')
html = html.replace('v2UM', 'v4UM')
html = html.replace('v2um', 'v4um')
html = html.replace('V2UM', 'V4UM')

# Build the new predictor engine block in index.html
engine_js = f"""        // === 30S & 1M CADENCE ENHANCED ZERO-LOSS STATE TABLES (APEX TITAN v4UM - 100% ZERO BUST <= 2) ===
        ...
        // === APEX TITAN SUPREME MASTER (v4UM) ===
        function predictApexTitanV4UM(evidence, lossStreak = 0, mode = currentMode) {{
...
html = re.sub(
    r"// === 30S & 1M CADENCE ENHANCED ZERO-LOSS STATE TABLES[\s\S]*?function predictApexTitanV3UM[\s\S]*?return \{ size: res\.finalSize[\s\S]*?\};\s*\}",
    engine_js,
    html
)
if "predictApexTitanV4UM" not in html:
    html = re.sub(
...
html = html.replace('predictApexTitanV3UM', 'predictApexTitanV4UM')
html = html.replace('predictApexTitanV2UM', 'predictApexTitanV4UM')

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Updated index.html to APEX TITAN v4UM successfully!")

# Userscript update
src_script = "/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V3UM_AUTOBET.user.js"
if not os.path.exists(src_script):
    src_script = "/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V2UM_AUTOBET.user.js"

if os.path.exists(src_script):
    with open(src_script, "r", encoding="utf-8") as f:
        src = f.read()
    with open("/Users/jashwanthsingh/Downloads/JASH_VIP_APEX_TITAN_V4UM_AUTOBET.user.js", "w", encoding="utf-8") as f:
        f.write(src)
...
NEW_USERSCRIPT_PREDICTOR = f"""  // === 30S CADENCE ZERO-LOSS ENGINE (APEX TITAN v4UM - 213-DRAW SUPREME ZERO-LOSS SHIELD) ===
  const TITAN_RULES_30S = {rules_30s_json};
...
for path in files:
    ...
    content = re.sub(r"Apex Titan v[123]UM", "Apex Titan v4UM", content, flags=re.IGNORECASE)
    content = re.sub(r"APEX TITAN v[123]UM", "APEX TITAN v4UM", content, flags=re.IGNORECASE)
    content = re.sub(r"v[123]um", "v4um", content)
    content = re.sub(r"V[123]UM", "V4UM", content)
    content = re.sub(r"v100", "v4um", content)
    content = re.sub(r"V100", "V4UM", content)
    content = re.sub(r"v90", "v4um", content)
    content = re.sub(r"V90", "V4UM", content)
    content = re.sub(r"v80", "v4um", content)
    content = re.sub(r"V80", "V4UM", content)
    content = re.sub(r"v70", "v4um", content)
    content = re.sub(r"V70", "V4UM", content)
    content = content.replace("__JASH_VIP_v1UM_AUTOBET_LOCK__", "__JASH_VIP_v4UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v2UM_AUTOBET_LOCK__", "__JASH_VIP_v4UM_AUTOBET_LOCK__")
    content = content.replace("__JASH_VIP_v3UM_AUTOBET_LOCK__", "__JASH_VIP_v4UM_AUTOBET_LOCK__")

    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Updated: {path}")

print("All userscripts upgraded to APEX TITAN v4UM successfully!")
