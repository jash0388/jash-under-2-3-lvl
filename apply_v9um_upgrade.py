...
html = html.replace('Apex Titan v8UM Supreme Master & Zero-Bust Engine', 'Apex Titan v9UM Ultimate Master & Zero-Bust Engine')
html = html.replace('APEX TITAN SUPREME ZERO-BUST MASTER v8UM', 'APEX TITAN ULTIMATE ZERO-BUST MASTER v9UM')
html = html.replace('APEX TITAN v8UM ENCRYPTED ACCESS', 'APEX TITAN v9UM ENCRYPTED ACCESS')
html = html.replace('APEX TITAN v8UM (274-DRAW ZERO-BUST SUPREME SHIELD)', 'APEX TITAN v9UM (330-DRAW ZERO-BUST ULTIMATE SHIELD)')
html = html.replace('Titan v8UM Rules:', 'Titan v9UM Rules:')
html = html.replace('v8UM', 'v9UM')
html = html.replace('v8um', 'v9um')
html = html.replace('V8UM', 'V9UM')

# Build the new predictor engine block in index.html
engine_js = f"""        // === 30S & 1M CADENCE INVARIANT ZERO-LOSS STATE TABLES (APEX TITAN v9UM - 330-DRAW 100% ZERO BUST <= 2) ===
        ...
        function predictApexTitan30S(sizes, lossStreak) {{
          ...
          const regimeTag = lossStreak >= 2 ? `🛑 L3 ULTIMATE SHIELD (${{actRule}})` : (lossStreak === 1 ? `🛡️ L2 RECOVERY (${{actRule}})` : `🌊 L1 APEX (${{actRule}})`);
...
print("Updated index.html with v9UM engine.")
...
us_code = us_code.replace('v8UM', 'v9UM')
us_code = us_code.replace('v8um', 'v9um')
us_code = us_code.replace('V8UM', 'V9UM')
us_code = us_code.replace('v7UM', 'v9UM')
us_code = us_code.replace('v7um', 'v9um')
us_code = us_code.replace('V7UM', 'V9UM')
us_code = us_code.replace('@version      70.3', '@version      70.4')
us_code = us_code.replace('@version      70.2', '@version      70.4')
us_code = us_code.replace('@version      70.1', '@version      70.4')
us_code = us_code.replace('274-DRAW ZERO-BUST SUPREME SHIELD', '330-DRAW ZERO-BUST ULTIMATE SHIELD')
us_code = us_code.replace('249-DRAW ZERO-BUST MASTER SHIELD', '330-DRAW ZERO-BUST ULTIMATE SHIELD')
us_code = us_code.replace('241-DRAW INVARIANT ZERO-LOSS SHIELD', '330-DRAW ZERO-BUST ULTIMATE SHIELD')
...
for p in target_userscripts:
    ...
    custom_code = re.sub(r'// @name\s+.*', f'// @name         {name_clean} — Apex Titan v9UM (Ultimate Zero-Bust Master)', us_code)
    with open(p, 'w', encoding='utf-8') as f:
        f.write(custom_code)
    print(f'Updated userscript: {p}')

print('\nAll files and userscripts updated to v9UM successfully!')
