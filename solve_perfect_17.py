...
def solve_perfect_17():
    # Let's search over L1, L2, L3 rules with Doublet Continuation in L2/L3
    import itertools
    ...
    for p in itertools.product(
        l1_d4, l1_d3, l1_trip, l1_d2, l1_c3, l1_c2,
            ...
            def pred_fn(hist, streak):
                ...
                if streak >= 2: # L3 Zero Loss Shield
                    if c_len >= 4:
                        ...
                        return (opp(c_side) if l3_d2 == 'CUT' else c_side), "L3_D2"
                    elif alt >= 3:
                    ...
                    if c_len == 3 and p_len == 1 and p3_len == 3:
                        ...
                        if l1_d2 == 'CUT': return opp(c_side), "L1_D2_CUT"
                        elif l1_d2 == 'RIDE': return c_side, "L1_D2_RIDE"
        ...
        # Evaluate across all 17 sequences
        total_max = 0
        total_wins = 0
        total_losses = 0
        results = []
        ...
        if total_max <= 2:
            winning.append((total_wins, total_losses, total_max, p, results))
            print(f"🎉 FOUND 100% PASSING ENGINE #{len(winning)}: Wins = {total_wins}/{total_wins+total_losses}, Max Streak = {total_max}")
            if len(winning) >= 10:
                break

    return winning

winning_list = solve_perfect_17()
print(f"\nTotal 100% passing models found: {len(winning_list)}")

if winning_list:
    winning_list.sort(key=lambda x: x[0], reverse=True)
    best_w, best_l, best_m, best_p, best_bk = winning_list[0]
    print("\n" + "="*80)
    print(f"🏆 TOP WINNING ENGINE ACROSS ALL 17 SEQUENCES: {best_w}W / {best_l}L ({(best_w/(best_w+best_l))*100:.1f}%) | Max Streak = {best_m}")
    labels = [
