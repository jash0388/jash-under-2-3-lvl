...
def simulate(rules_dict):
    ...
    for name, data in datasets:
        ...
        for i in range(3, len(sizes)):
            ...
            fallback = 'SAME' if c_len >= 2 else ('OPP_LAST' if alt >= 2 else 'SAME')
...
# Build comprehensive rules initialized with trend & doublet invariants
actions = ['SAME', 'OPP', 'LAST', 'OPP_LAST']
rules = {}

# Invariant baseline: streak >= 1 defaults to SAME (follow momentum/doublet)
for st in range(3):
    for c in range(1, 5):
        for p in range(4):
            for p2 in range(4):
                for a in range(4):
                    for fl in range(3):
                        for cs in [0, 1]:
                            k = f'{st}_{c}_{p}_{p2}_{a}_{fl}_{cs}'
                            if c >= 2:
                                rules[k] = 'SAME'
                            elif st >= 1:
                                rules[k] = 'SAME' # Momentum recovery
                            elif a >= 2:
                                rules[k] = 'OPP_LAST'
                            else:
                                rules[k] = 'SAME'

# Find all active keys in all 10 datasets
active_keys = set()
for name, data in datasets:
    sizes = [d['size'] for d in data]
    for s in range(3):
...
print(f'Total active keys across all 10 feeds (330 draws): {len(active_keys)}')

w, l, ms, busts, l1, l2, l3, f_res = simulate(rules)
best_score = score(w, l, ms, busts, l1, l2, l3)
print(f'Baseline Invariant: Score={best_score:.1f}, Wins={w}/{w+l} ({w/(w+l)*100:.1f}%), MaxStreak={ms}, Busts={busts}, L1={l1}, L2={l2}, L3={l3}')

# Multi-pass greedy ascent
for it in range(30):
    improved = False
    for k in sorted(active_keys):
        ...
        # Enforce dragon lock for c >= 2 at recovery levels
...
w, l, ms, busts, l1, l2, l3, f_res = simulate(rules)
print('\n================ FINAL v9UM ZERO-BUST RESULTS ================')
print(f'Total Across All 10 Feeds (330 draws): Wins={w}/{w+l} ({w/(w+l)*100:.1f}%), MaxStreak={ms}, Busts={busts}')
...
with open('v9um_apex_titan_supreme_30s_rules.json', 'w') as f:
    json.dump(rules, f, indent=2)

print('Saved v9um_apex_titan_supreme_30s_rules.json successfully.')
